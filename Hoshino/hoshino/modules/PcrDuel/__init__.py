...
[查询贵族] 可以查询自己的贵族状态。
[贵族约会/招募女友] 可以花费300金币招募一个女友，女友数不能超过爵位上限。
[升级贵族/升级爵位] 可以花费金币提升自己的爵位,提升爵位时女友数需已达到上限。
[贵族决斗+艾特] 可以向另一个贵族发起决斗，两个人必须都是贵族且已拥有女友，输者会随机被抢走一个女友。
[领金币] 可以在金币归零时领取50金币。
[查金币] 可以查询自己现在的金币数。
[贵族签到] 可以每天一次的领取100金币。
[为(用户qq)充值xx金币] 可以为该用户充值金币。例如 为123456充值100金币(该功能仅限机器人管理员，可以自己修改权限)
[重置决斗] 可以解锁决斗开关。(限管理员以上，这个仅用于决斗卡死无法再开的时候)
[查女友+PCR角色名] 可以查询某位pcr角色的归属。
""".strip()

lmt = DailyNumberLimiter(1)
sv = Service('pcr-duel', enable_on_default=True, bundle='pcr娱乐',help_=HELP_MSG)
